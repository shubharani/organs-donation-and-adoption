 ...
 if (preg_match("$([0-9]{4})-([0-9]{2})-([0-9]{2})$",$RegisteredDate))
{
	...
	if($count == 1 && $row['password']==$password)
	{
		$_SESSION['loginid'] = $row['loginid'];
               $sql = "SELECT COUNT(*) FROM hni WHERE loginid LIKE '$loginid'";
	...
	{	
          ...
          $insertSQL = "INSERT INTO hni VALUES('$Firstname','$Lastname','$loginid','$Age','$DateOfBirth','$Gender','$Address','$MobileNumber1','$MobileNumber2','$BloodGroup','$Disease','$RegisteredDate')";
		...
		if($insertSQL)
		{
			...
			echo "<script>window.location.href='hd.php'</script>";
                }
             else
		{
           ...
           else
	{
		echo "<script>alert('you are already registered using this loginid!');</script>";
   		...
   		echo "<script>window.location.href='h1.html'</script>";
	}
	}
   ...
   {
     echo "<script>alert('The dateofbirth format is: YYYY-MM-DD !');</script>";
    echo "<script>window.location.href='h1.html';</script>";
    }
   ...
   {
     echo "<script>alert('The dateofbirth and RegisteredDate format is: YYYY-MM-DD !');</script>";
    echo "<script>window.location.href='h1.html';</script>";
    }
